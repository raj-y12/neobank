...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const recipient = data.recipient && typeof data.recipient === "object" ? data.recipient as { name?: unknown; accountMask?: unknown } : {};
    return NextResponse.json({ standingOrder: { ...data, recipient: { name: typeof recipient.name === "string" ? recipient.name : "Unknown recipient", accountMask: typeof recipient.accountMask === "string" ? recipient.accountMask : null } } });
  } catch (error) {
