...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!process.env.INCREASE_API_KEY || !process.env.INCREASE_ACCOUNT_ID) return NextResponse.json({ error: "Increase sandbox is not configured" }, { status: 409 });
    ...
    return NextResponse.json({ accepted: true, providerStatus: result.status }, { status: 202 });
  } catch (error) {
