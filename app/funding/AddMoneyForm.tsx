...
export function AddMoneyForm({ enabled }: { enabled: boolean }) {
  ...
  return <div className="funding-actions"><div className="form-row"><label>Amount in USD<input className="input" value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" disabled={!enabled || busy} /></label><button className="btn btn-primary" onClick={createFunding} disabled={!enabled || busy}>{busy && feedback?.status === "CREATING" ? "Adding…" : "Add money"}</button></div>{feedback && typeof document !== "undefined" && createPortal(<div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setFeedback(null)}><FundingFeedbackModal feedback={feedback} onClose={() => setFeedback(null)} onSimulateSettlement={() => simulate("SETTLED")} onSimulateReturn={() => simulate("RETURNED")} /></div>, document.body)}</div>;
}
